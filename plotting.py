...
def plot_autoencoder_stats(
    ...
    canvas = np.zeros((68 * rows, columns * 68))
    for i in range(rows):
        for j in range(columns):
            idx = i % columns + rows * j
            canvas[i * 68:(i + 1) * 68, j * 68:(j + 1) * 68] = x[idx].reshape((68, 68))
    ...
    for i in range(rows):
        for j in range(columns):
            idx = i % columns + rows * j
            canvas[i * 68:(i + 1) * 68, j * 68:(j + 1) * 68] = x_hat[idx].reshape((68, 68))
...
def plot_samples(ax, x):
    ...
    x_grid = make_grid(x.view(-1, 1, 68, 68), nrow=nrow).permute(1, 2, 0)
    ax.imshow(x_grid)
    ax.axis('off')
